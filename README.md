...
### tech-lead
Orchestrates the team. Breaks down features, dispatches subagents, reviews plans, resolves blockers. Entry point for complex tasks.

### dev-rails
Senior Rails developer. Implements features and bugfixes following Rails conventions. Owns branches, commits, and PRs. Runs tests. Does not create issues.
...
1. **tech-lead** receives a feature request → breaks it into tasks → dispatches to agents
2. **dev-rails** implements → opens PR
3. **rails-reviewer** reviews the PR → requests changes or approves
4. **qa** validates behavior against requirements
5. **product-owner** closes the related issue on merge
